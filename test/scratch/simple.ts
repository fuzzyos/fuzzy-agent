...
const { skills: sourcedSkills } = await loadSourcedSkills<Source, SourcedSkill>(
	env,
	[
		source("project", join(env.cwd, ".fuzzy/skills")),
		source("user", join(homedir(), ".fuzzy/agent/skills")),
		source("path", join(env.cwd, "../../../fuzzy-skills")),
	],
	(skill, source) => ({ ...skill, source }),
);
const { promptTemplates: sourcedPromptTemplates } = await loadSourcedPromptTemplates<Source, SourcedPromptTemplate>(
	env,
	[source("project", join(env.cwd, ".fuzzy/prompts")), source("user", join(homedir(), ".fuzzy/agent/prompts"))],
