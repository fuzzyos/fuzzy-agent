...
# Fuzzy Observability Design Notes

## Goal

Make `packages/ai` and `packages/agent`/harness observable without depending on OpenTelemetry, Sentry, or any APM vendor.

Fuzzy should emit stable, structured lifecycle events. External listeners can convert those events into OTel spans, Sentry spans, logs, metrics, or custom telemetry.
...
```text
traceId=t1 spanId=s1 parent=-  name=fuzzy.agent.prompt
traceId=t1 spanId=s2 parent=s1 name=fuzzy.agent.turn
traceId=t1 spanId=s3 parent=s2 name=fuzzy.ai.provider.request
traceId=t1 spanId=s4 parent=s2 name=fuzzy.agent.tool_call
traceId=t1 spanId=s5 parent=s4 name=fuzzy.session.append_entry
```
...
Fuzzy must run in Node, Bun, browser, workers, and other JS runtimes, so ALS cannot be the core abstraction. It should be a runtime adapter.

## Core design

Fuzzy owns a small runtime-agnostic observability abstraction:
...
channel("fuzzy.observability").publish(event);
```

Subscribers can create OTel/Sentry spans without monkey-patching fuzzy.

## What fuzzy emits

Fuzzy emits what happened. It does not create OTel/Sentry spans directly.
...
fuzzy.agent.prompt
fuzzy.agent.skill
fuzzy.agent.prompt_template
fuzzy.agent.compaction
fuzzy.agent.branch_navigation
fuzzy.agent.session.append_entry
fuzzy.ai.provider.request
```
...
```text
fuzzy.agent.turn
fuzzy.agent.tool_call
fuzzy.agent.queue_update
fuzzy.ai.provider.retry
fuzzy.ai.provider.first_token
fuzzy.ai.provider.usage
fuzzy.session.read
fuzzy.session.write
```
...
Observability must never affect fuzzy execution.
...
{
  type: "start",
  name: "fuzzy.ai.provider.request",
  traceId: "t1",
  spanId: "s3",
  parentSpanId: "s1",
  context: {
...
```text
packages/ai
  emits fuzzy.ai.* events

packages/agent
  emits fuzzy.agent.* / fuzzy.session.* events
```
...
packages/otel
  subscribes to fuzzy events and creates OpenTelemetry spans
```

## Thesis

Fuzzy defines a stable, safe event contract. Adapters define where events go.
